...
contract Perpetual is MarginAccount, ReentrancyGuard {
     ...
     */
    function pause(uint32 pausedDuration) external {
        require(globalConfig.pauseControllers(msg.sender), "unauthorized caller");
        require(pausedDuration <= 7 days, "paused duration too long");
        pausedExpiration = uint64(block.timestamp + pausedDuration);
     ...
     */
    function disableWithdraw(uint32 disabledDuration) external {
        require(globalConfig.withdrawControllers(msg.sender), "unauthorized caller");
        require(disabledDuration <= 7 days, "disable withdraw duration too long");
        disableWithdrawExpiration = uint64(block.timestamp + disabledDuration);
        emit DisableWithdraw(msg.sender,disableWithdrawExpiration);
    }
     ...
     * @return True if give trader is safe with initial margin rate.
     */
    function liquidate(address trader, uint256 maxAmount) external onlyNotPaused returns (uint256, uint256) {
        ...
        depositImplementation(
            msg.sender,
            toCollateral(liquidationPrice.wmul(liquidationAmount).wmul(governance.initialMarginRate).toInt256())
        );
        if (opened > 0) {
    ...
    function tradePosition(
        ...
        (tradeData.takerOpened, tradeData.takerClosed, tradeData.takerOriginalSize) = MarginAccount.trade(
            taker,
            side,
            price,
            amount
        );
        (tradeData.makerOpened, tradeData.makerClosed, tradeData.makerOriginalSize) = MarginAccount.trade(
            maker,
            LibTypes.counterSide(side),
            price,
            amount
        );
