...
contract Exchange {
    ...
    function matchOrders(
        ...
        for (uint256 i = 0; i < makerOrderParams.length; i++) {
            ...
            require(
                orderDatas[i].amount <= makerOrderParams[i].amount.sub(makerFilledAmount),
                mergeS1AndS2ReturnString(uint2str(i), ":maker overfilled")
            );
            require(
                orderDatas[i].amount.mod(perpetual.getGovernance().tradingLotSize) == 0,
                "amount must be divisible by tradingLotSize"
            );
    ...
    function fillOrder(
        ...
        require(
            perpetual.isSafe(makerOrderParam.trader),
            mergeS1AndS2ReturnString(uint2str(orderData.index), ":maker unsafe")
        );
    ...
    function validateOrderParam(
        ...
        require(
            orderParam.orderVersion() == SUPPORTED_ORDER_VERSION,
            mergeS1AndS2ReturnString(index, ":unsupported version")
        );
        ...
        require(
            ...
            mergeS1AndS2ReturnString(index, ":invalid signature")
        );
    ...
    function dealOrderData(
        ...
        if (opened > 0) {
            ...
            //sub position
            traderMinimumBalance = (originalSize.sub(closed))
                .wdiv(originalSize)
                .wmul(traderMarginBalance.toUint256())
                .toInt256()
                .add(fee);
        }

        if (traderMarginBalance > traderMinimumBalance) {
            perpetual.withdrawFor(
                payable(trader),
                (traderMarginBalance.sub(traderMinimumBalance)).div(perpetual.scaler()).toUint256()
            );
        } else if (traderMarginBalance < traderMinimumBalance) {
            perpetual.depositFor(
                trader,
                (traderMinimumBalance.sub(traderMarginBalance)).div(perpetual.scaler()).toUint256()
            );
        }
    }

    function mergeS1AndS2ReturnString(string memory s1, string memory s2) internal pure returns (string memory) {
